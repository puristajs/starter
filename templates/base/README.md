...
Attached agents keep model, sandbox, durable runtime, and durable workspace stores in service bootstrap/config via `ai.models`, `ai.sandbox`, `ai.runtime`, and `ai.workspaceStore`. Declare durable replay with `setWorkspacePolicy({ mode: 'durable', required: true })` only when an agent must resume from committed workspace checkpoints; otherwise agents remain ephemeral by default.
